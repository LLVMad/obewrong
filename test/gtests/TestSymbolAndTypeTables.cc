...
TEST(SymbolTableTest, AddSymbol) {
  ...
  auto varDecl =
      std::make_unique<VarDecl>("x", std::make_unique<TypeInt>(), nullptr);
  bool added = symTable.addSymbol("x", std::move(varDecl));
  EXPECT_TRUE(added);

  // Тест 2: Повторное объявление
  auto varDecl2 =
      std::make_unique<VarDecl>("x", std::make_unique<TypeReal>(), nullptr);
  ...
  symTable.enterScope();
  auto localVar =
      std::make_unique<VarDecl>("y", std::make_unique<TypeBool>(), nullptr);
...
TEST(TypeTableTest, AddClassArrayFunction) {
  ...
  EXPECT_TRUE(
      typeTable.addArrayType(arrayName, std::make_unique<TypeInt>(), 10));
