...
//   auto fromType = fromExpr->resolveType(typeTable->types[moduleName],
//   currentScope); auto toType = node.to;
...
//     indexVal = builder->CreateSExt(indexVal,
//     llvm::Type::getInt64Ty(*context));
//   }

//   // array type
//   // @TODO: field as `arr`
//   auto [arrDecl , arrAlloca, arrInited ] =
//   *currentScope->getSymbol(node.arr->getName());
...
//   auto [decl, alloca, isInited] =
//   *currentScope->getSymbol(node.obj->getName());

//   auto varType = decl->resolveType(typeTable->types[moduleName],
//   currentScope);
...
//     //   auto [decl, alloca, isInited] =
//     *currentScope->getSymbol(fieldRef->obj->getName());
...
//     //   auto arrType =
//     std::static_pointer_cast<VarDecl>(currentScope->lookup(elementRef->arr->getName()))->type;
//     //   auto el_type =
//     std::static_pointer_cast<TypeArray>(arrType)->el_type;
//     //
//     //   val = builder->CreateLoad(
//     //     el_type->toLLVMType(*context),
//     //     val
...
//   llvm::Type *classType = llvm::StructType::getTypeByName(*context,
//   node.left->getName());

//   // @TODO
//   if (!classType) {
//     classType = typeTable->getType(moduleName,
//     "Integer")->toLLVMType(*context);
//   }
...
//     typeNames += Args[i]->resolveType(typeTable->types[moduleName],
//     currentScope)->name;
...
//     case 8: {
//       lastValue =
//       llvm::ConstantInt::getSigned((llvm::Type::getInt8Ty(*context)),
//                                     node.getValue());
//       break;
//     }
//     case 16: {
//       lastValue =
//       llvm::ConstantInt::getSigned((llvm::Type::getInt16Ty(*context)),
//       node.getValue()); break;
//     }
//     case 32: {
//       lastValue =
...
//   lastValue = llvm::ConstantFP::get(*context,
//   llvm::APFloat(node.getValue()));
// }

// void CodeGenVisitor::visit(ArrayLiteralExpr &node) {
//   auto elType = typeTable->getType(moduleName, node.el_type);
//   auto arrayTypeLLVM = llvm::ArrayType::get(elType->toLLVMType(*context),
//   node.elements.size());
...
//     node.elements.size() *
//     elType->toLLVMType(*context)->getPrimitiveSizeInBits() / 8);

//   // cast to i8*
//   auto destPtr = builder->CreateBitCast(localArray,
//   llvm::PointerType::get(llvm::Type::getInt8Ty(*context), 0)); auto srcPtr =
//   builder->CreateBitCast(globalArray,
...
//     llvm::Type::getInt64Ty(*context)
//   };
//   auto memcpyFn = getOrInsertDeclaration(module.get(),
//   llvm::Intrinsic::memcpy, types); builder->CreateCall(memcpyFn, {destPtr,
//   srcPtr, size, llvm::ConstantInt::getFalse(*context)});

//   lastValue = localArray;
// }

// void CodeGenVisitor::visit(VarRefEXP &node) {
//   // llvm::AllocaInst *alloca = currentScope->lookupAlloca(node.getName());
//   /* varEnv[node.getName()]; */
...
//   lastValue = builder->CreateLoad(alloc->getAllocatedType(), alloc,
//   node.getName().c_str());
// }
...
//   auto classType = llvm::StructType::create(*context,
//   llvm::StringRef(node.getName()));
...
//     // typeNames += arg->resolveType(typeTable->types[moduleName],
//     currentScope)->name;
//   }
...
//     && currentScope->getName() != node.getName()) currentScope =
...
//     initializer->getKind() != E_Array_Literal) {

//       // if initVal is a pointer (EL_REF, GEP)
//       llvm::Value* initValUnwrap = unwrapPointerReference(initializer.get(),
//       initVal);
...
//       alloca = dyn_cast<llvm::AllocaInst>(initVal); // @TODO look into
//       ALLOCAJUMP
//     }
//   }
...
//   llvm::Function *F = llvm::Function::Create(FT,
//     (node.getKind() == E_Main_Decl) ? llvm::Function::ExternalLinkage :
...
//   auto type = node.condition->resolveType(typeTable->types[moduleName],
//   currentScope);
...
//   PHI. ElseBB = builder->GetInsertBlock();
...
//   auto [iterDecl, iterAlloca, iterInited] =
//   *currentScope->getSymbol<VarDecl>(iteratorVar->getName()); auto
//   iteratorType = iterDecl->type; auto iterTypeLLVM =
//   iteratorType->toLLVMType(*this->context);

//   // loop header block
//   llvm::Function *TheFunction = builder->GetInsertBlock()->getParent();
//   llvm::BasicBlock *PreheaderBB = builder->GetInsertBlock();
//   llvm::BasicBlock *LoopBB = llvm::BasicBlock::Create(*context, "loop",
//   TheFunction); llvm::BasicBlock *AfterBB =
...
//   llvm::PHINode *Variable = builder->CreatePHI(iterTypeLLVM, 2,
//   iteratorVar->getName());
...
//   switch (node.assKind) {
//   case VAR_ASS: {
//     auto [_, alloc, isInited] =
//     *currentScope->getSymbol(node.variable->getName()); var = alloc; name =
//     node.variable->getName(); currentScope->markInitialized(name); break;
//   }
//   case FIELD_ASS:
...
//   auto leftType = node.left->resolveType(typeTable->types[moduleName],
//   currentScope); auto className = leftType->name;
...
//     } else if (className == "Real") {
//       auto negOne = llvm::ConstantFP::get(llvm::Type::getFloatTy(*context),
//       -1.0); lastValue = builder->CreateFMul(L, negOne, "fuminus");
//     }
//   }
// }
...
//       auto classBTScope = child->getChildren()[0]; // ? single module -
//       single class : Integer - Integer decl =
...
//       auto arrType =
//       currentScope->lookup<VarDecl>(elementRef->arr->getName())->type; auto
...
//       auto [varDecl, alloca, isInited] =
//       *currentScope->getSymbol<VarDecl>(fieldRef->obj->getName());

//       auto type = varDecl->resolveType(typeTable->types[moduleName],
//       currentScope); if (type->kind == TYPE_ACCESS) {
