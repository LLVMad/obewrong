...
enum TypeKind {
  ...
  TYPE_CLASS,
  TYPE_LIST,
  TYPE_FUNC, // important to distinguish, this not a type as in types in a reference manual, this a type of an ENTITY, function itself has a signature
  TYPE_BOOL,
  TYPE_GENERIC,
};
...
public:
  ...
  TypeFunc(std::unique_ptr<Type> return_type, std::vector<std::unique_ptr<Type>> args)
    : Type(TYPE_FUNC, "Function"), return_type(std::move(return_type)), args(std::move(args)) {}
};

// class "signature"
// is a:
// [base_class] -> field_types[0] -> field_types[1] -> ... -> method_types[0] -> method_types[1] -> ...
...
//  std::string name;
  ...
  TypeClass(const std::string &name, std::vector<std::unique_ptr<Type>> fields_types, std::vector<std::unique_ptr<TypeFunc>> methods_types)
    : Type(TYPE_CLASS, name), fields_types(std::move(fields_types)), methods_types(std::move(methods_types)) {}
};
