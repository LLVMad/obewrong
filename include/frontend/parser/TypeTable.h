...
public:
  ...
  bool addClassType(const std::string &className) {
    ...
    addType(className, std::make_unique<TypeClass>(
                           className, std::vector<std::unique_ptr<Type>>(),
                           std::vector<std::unique_ptr<TypeFunc>>()));
    return true;
  }

  bool addArrayType(const std::string &name, std::unique_ptr<Type> elementType,
                    uint32_t size) {
    if (exists(name)) {
      return false;
    }
  ...
  bool addFuncType(const std::string &name, std::unique_ptr<Type> returnType,
                   std::vector<std::unique_ptr<Type>> args) {
    if (exists(name)) {
      return false;
    }
    addType(name,
            std::make_unique<TypeFunc>(std::move(returnType), std::move(args)));
    return true;
  }
  ...
  bool exists(const std::string &name) const { return types.count(name); }
};
